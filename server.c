...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            //define a pointer and index to the first space
            char* space_1_ptr = strchr(line,' '); 
            int space_1_index;
            ...
            //determine index of space_1
            else            
            {
                space_1_index = space_1_ptr - line;
            }            
            
            //define a pointer and index to the second space
            char* space_2_ptr = NULL;
            if (space_1_ptr != NULL)
            {
             space_2_ptr = strchr(space_1_ptr + 1,' ');
            }
            int space_2_index;
            //check if NULL
            if (space_2_ptr == NULL)
            ...
            //determine index of space_2_ptr
            else
            {
                space_2_index = space_2_ptr - line;
            }            
               
            //ensure there is no third space
            if (space_1_ptr != NULL && space_2_ptr != NULL)
            {
                if (strchr(space_2_ptr + 1,' ') != NULL)
                {
                    printf("400 Bad Request");    
                }
            }
                       
            //define a pointer and index for CRLF substring
            char* CRLF_ptr = NULL;
            int CRLF_index;
            //check if CRLF is NULL
            if (strcmp(line + line_len - 2, "\r\n") != 0)
            {
                printf("400 Bad Request");
            }
            //determine CRLF_index of CRLF_ptr
            else
            {
                CRLF_ptr = line + line_len - 2;
                CRLF_index = CRLF_ptr - line;
            }
            
            //define method
            char method[line_len];
            if (space_1_ptr != NULL)
            {
                for (int i = 0; i < space_1_index; i++)
                {
                    method[i] = line[i];
                }
                method[space_1_index] = '\0';
            }   
            
            //define request-target
            char request_target[line_len];
            if (space_1_ptr != NULL && space_2_ptr != NULL)
            {
                for (int i = 0; i < (space_2_index - space_1_index); i++)
                {
                    request_target[i] = line[i + space_1_index + 1];
                }
                request_target[space_2_index - space_1_index - 1] = '\0';
            }
            
            //define a pointer and index for ? in request target
            char* questionmark_ptr = strchr(request_target,'\?');
            int questionmark_index;
            // redefine index if pointer not NULL
            if (questionmark_ptr != NULL)
            {
                questionmark_index = questionmark_ptr - line;
            }
            
            //define abs_path
            char abs_path[line_len];
            if (questionmark_ptr == NULL && space_1_ptr != NULL && space_2_ptr != NULL)
            {
                for (int i = 0; i < (space_2_index - space_1_index); i++)
                {
                    abs_path[i] = line[i + space_1_index + 1];
                }
                abs_path[space_2_index - space_1_index - 1] = '\0';
            }
            else if (questionmark_ptr != NULL && space_1_ptr != NULL && space_2_ptr != NULL)
            {
                for (int i = 0; i < questionmark_index - space_1_index; i++)
                {
                    abs_path[i] = line[i + space_1_index + 1];
                }
                abs_path[questionmark_index - space_1_index - 1] = '\0';
            }
            
            //define HTTP_version
            char HTTP_version[line_len];
            //assign characters from line array
            if (space_2_ptr != NULL && CRLF_ptr != NULL)
            {
                for (int i = 0; i < (CRLF_index - space_2_index); i++)
                {
                    HTTP_version[i] = line[(i + space_2_index + 1)];
                }
                HTTP_version[(CRLF_index - space_2_index) - 1] = '\0';
            }
            ...
            //check if request target beings with /
            if (strncmp(request_target,"/", sizeof(request_target[0])) != 0)
            {
                printf("501 Not Implemented");
            }
            
            //check if HTTP_version is HTTP/1.1
            ...
            //check if abs_path contains a .
            if (strchr(abs_path,'.') == NULL)
            {
                printf("501 Not Implemeted");
            }                    
            ...
            //define query
            char query[line_len];
            if (questionmark_ptr == NULL || questionmark_ptr == (space_2_ptr - 1))
            {
                query[0] = '\0';
            }
            else
            {
                for (int i = 0; i < (space_2_index - questionmark_index); i++)
                {
                    query[i] = line[i + questionmark_index + 1];
                }
                query[space_2_index - questionmark_index - 1] = '\0';
            }
            ...
            //define '.' ptr and index
            char* extension_ptr = strchr(line, '.');
            
            if (extension_ptr != NULL)
            {
                strncpy(extension, extension_ptr + 1, space_2_ptr - extension_ptr); 
            }
...
bool error(unsigned short code)
{
    ...
    // http://www.w3.org/Prothttp://www.http.com/?ga=lvKw0MOS0Y7X6mxZgIFICGZmyTleehgCqe0zYvNpfPs1IoaVNIrIT6uQ%2BdIE%2FMU7Atss4I0HQ8jq8johYAUy%2Fg%3D%3D&gerf=8qBxpfqluwoNK4bCXHwUfqpnPcPyJ%2FCmBAwxiMKRbdc%3D&guro=KW6%2B%2B%2FgyiVpXTkkmP1IETTk5xKt%2FQ5RYCk639w%2BPYQRX1lkRM%2FqF%2B0sH0jxferao&ocols/rfc2616/rfc2616-sec6.html#sec6.1
